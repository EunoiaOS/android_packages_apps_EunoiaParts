...
import eunoiaos.preference.EunoiaGlobalSettingListPreference;
...
public class TrustPreferences extends SettingsPreferenceFragment {
    ...
    private EunoiaGlobalSettingListPreference mUsbRestrictorPref;
    ...
    private void setup() {
        ...
        int currentFeatures = EunoiaSettings.Secure.getInt(getContext().getContentResolver(),
                EunoiaSettings.Secure.TRUST_WARNINGS, TrustInterface.TRUST_WARN_MAX_VALUE);
    ...
    private void setupEncryption(int level) {
        ...
        boolean isLegacy = getContext().getResources()
                .getBoolean(com.eunoiaos.platform.internal.R.bool.config_trustLegacyEncryption);
        if (level == TrustInterface.TRUST_FEATURE_LEVEL_GOOD) {
    ...
    private boolean onWarningChanged(Boolean value, int feature) {
        int original = EunoiaSettings.Secure.getInt(requireContext().getContentResolver(),
                EunoiaSettings.Secure.TRUST_WARNINGS, TrustInterface.TRUST_WARN_MAX_VALUE);
        int newValue = value ? (original | feature) : (original & ~feature);
        boolean success = EunoiaSettings.Secure.putInt(requireContext().getContentResolver(),
